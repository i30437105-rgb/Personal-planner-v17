...
import { Clock, Play, Square, Plus, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Star, Settings, Edit3, X, Check, Trash2, BarChart2, PieChart, AlertCircle, Archive, TrendingUp } from 'lucide-react';
...
const CHART_TYPES = [
  { id: 'bar', icon: BarChart2, label: 'Столбцы' },
  { id: 'pie', icon: PieChart, label: 'Круговая' },
  { id: 'trend', icon: TrendingUp, label: 'Тренд' },
];
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
      <div style={{ position: 'relative', width: '120px', height: '120px', flexShrink: 0 }}>
        ...
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
          <p style={{ fontSize: '14px', fontWeight: '600', color: COLORS.text }}>{formatMinutes(total)}</p>
        </div>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '6px' }}>
        {data.slice(0, 5).map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <span style={{ fontSize: '12px', color: COLORS.text, flex: 1 }}>{item.name}</span>
            <span style={{ fontSize: '12px', color: COLORS.textMuted }}>{formatMinutes(item.minutes)}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      {data.map((item, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            <span style={{ fontSize: '10px', color: COLORS.textMuted }}>{formatMinutes(item.value)}</span>
            <div style={{ width: '100%', height: '70px', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
              <div style={{ width: '100%', maxWidth: '30px', height: `${Math.max(height, 3)}%`, background: item.color || COLORS.gold, borderRadius: '4px 4px 0 0' }} />
            </div>
    ...
    <div style={{ padding: '10px 0' }}>
      <svg viewBox="0 0 100 100" style={{ width: '100%', height: '100px' }}>
        ...
        {data.map((d, i) => {
          const x = (i / (data.length - 1 || 1)) * 100;
          const y = 100 - (d.value / max) * 80;
          return <circle key={i} cx={x} cy={y} r="3" fill={COLORS.gold} />;
        })}
      </svg>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '8px' }}>
        {data.map((d, i) => <span key={i} style={{ fontSize: '10px', color: COLORS.textMuted }}>{d.label}</span>)}
      </div>
    </div>
  );
};
...
const ActivityForm = ({ existingActivity, onSave, onClose, onDelete, onArchive }) => {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      dailyGoal: !useWeeklyGoals && dailyGoal ? parseInt(dailyGoal) : null,
      weeklyGoals: useWeeklyGoals ? weeklyGoals.map(g => g ? parseInt(g) : null) : null,
      isArchived: existingActivity?.isArchived || false,
  ...
  return (
    <div style={{ maxHeight: '70vh', overflowY: 'auto' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        {!useWeeklyGoals ? (
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <input type="number" value={dailyGoal} onChange={(e) => setDailyGoal(e.target.value)} placeholder="0" style={{ ...inputStyle, flex: 1 }} />
            <span style={{ color: COLORS.textMuted, fontSize: '14px' }}>минут/день</span>
          </div>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '6px' }}>
            {WEEKDAYS.map((day, i) => (
              <div key={i} style={{ textAlign: 'center' }}>
                <span style={{ fontSize: '11px', color: COLORS.textMuted, display: 'block', marginBottom: '4px' }}>{day}</span>
                <input type="number" value={weeklyGoals[i]} onChange={(e) => { const newGoals = [...weeklyGoals]; newGoals[i] = e.target.value; setWeeklyGoals(newGoals); }} placeholder="0" style={{ width: '100%', padding: '8px 4px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '6px', color: COLORS.text, fontSize: '14px', textAlign: 'center', outline: 'none' }} />
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'flex', gap: '12px' }}>
        {existingActivity && (
          <>
            <button onClick={() => onArchive(existingActivity.id)} style={{ padding: '16px', background: `${COLORS.warning}20`, border: `1px solid ${COLORS.warning}50`, borderRadius: '12px', color: COLORS.warning, cursor: 'pointer' }}><Archive style={{ width: '18px', height: '18px' }} /></button>
...
const SessionForm = ({ session, activity, onSave, onClose, onDelete }) => {
  ...
  const handleSave = () => {
    ...
    onSave({ ...session, startAt: new Date(startTime).toISOString(), endAt: new Date(endTime).toISOString(), durationMinutes: duration });
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '12px' }}>
        ...
        <button onClick={onClose} style={{ flex: 1, padding: '16px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '12px', color: COLORS.text, fontSize: '15px', cursor: 'pointer' }}>Отмена</button>
        <button onClick={handleSave} disabled={duration <= 0} style={{ flex: 1, padding: '16px', background: duration > 0 ? COLORS.gold : COLORS.bgCard, border: 'none', borderRadius: '12px', color: duration > 0 ? COLORS.bg : COLORS.textDark, fontSize: '15px', fontWeight: '600', cursor: duration > 0 ? 'pointer' : 'not-allowed' }}>Сохранить</button>
      </div>
    </div>
  );
};
...
export const ProductivityScreen = ({ data, saveData }) => {
  ...
  useEffect(() => {
    if (activeSession) {
      timerRef.current = setInterval(() => setElapsedTime(Math.floor((Date.now() - new Date(activeSession.startAt).getTime()) / 1000)), 1000);
    }
  ...
  useEffect(() => {
    ...
    if (active) { setActiveSession(active); setElapsedTime(Math.floor((Date.now() - new Date(active.startAt).getTime()) / 1000)); }
  }, [sessions]);
  ...
  const startSession = (activityId, confirmed = false) => {
    ...
    if (activeSession) {
      newSessions = newSessions.map(s => s.id === activeSession.id ? { ...s, endAt: new Date().toISOString(), durationMinutes: Math.floor(elapsedTime / 60) } : s);
    }
  ...
  const stopSession = () => {
    ...
    saveData({ ...data, sessions: sessions.map(s => s.id === activeSession.id ? { ...s, endAt: new Date().toISOString(), durationMinutes } : s) });
  ...
  const handleSaveActivity = (activity) => {
    ...
    const newActivities = existingIndex >= 0 ? activities.map((a, i) => i === existingIndex ? activity : a) : [...activities, activity];
  ...
  const handleArchiveActivity = (activityId) => {
    saveData({ ...data, activities: activities.map(a => a.id === activityId ? { ...a, isArchived: true } : a) });
    setShowActivityForm(false);
    setEditingActivity(null);
  };

  const handleRestoreActivity = (activityId) => {
    saveData({ ...data, activities: activities.map(a => a.id === activityId ? { ...a, isArchived: false } : a) });
  };
  ...
  const handleSaveSession = (session) => {
    saveData({ ...data, sessions: sessions.map(s => s.id === session.id ? session : s) });
  ...
  const getDailyGoal = (activity, date) => {
    if (activity.weeklyGoals) {
      const dayIndex = getDayOfWeek(date);
      return activity.weeklyGoals[dayIndex] || 0;
    }
    return activity.dailyGoal || 0;
  };
  
  const dayStats = activeActivities.map(a => {
    ...
    const goal = getDailyGoal(a, selectedDate);
    return { ...a, totalMinutes, sessions: activitySessions, goal };
  ...
  const getChartData = () => {
    ...
    if (chartPeriod === 'today') {
      return activeActivities.filter(a => a.isFavorite || daySessions.some(s => s.activityId === a.id)).map(a => {
        const mins = daySessions.filter(s => s.activityId === a.id).reduce((sum, s) => sum + (s.durationMinutes || 0), 0);
        return { name: a.name, minutes: mins, color: a.color, label: a.icon, value: mins };
      });
      ...
      return WEEKDAYS.map((day, i) => {
        ...
        return { label: day, value: mins, color: COLORS.gold };
      });
    } else if (chartPeriod === 'month') {
      const weeks = []; const monthStart = getMonthStart(now);
      for (let w = 0; w < 4; w++) {
        let mins = 0;
        for (let d = 0; d < 7; d++) {
          const date = new Date(monthStart); date.setDate(date.getDate() + w * 7 + d);
          if (date.getMonth() === now.getMonth()) {
            const key = formatDateKey(date);
            mins += sessions.filter(s => s.date === key && s.endAt).reduce((sum, s) => sum + (s.durationMinutes || 0), 0);
          }
        }
        weeks.push({ label: `Нед ${w + 1}`, value: mins, color: COLORS.gold });
      }
      return weeks;
    } else if (chartPeriod === 'custom' && customDateFrom && customDateTo) {
      const from = new Date(customDateFrom); const to = new Date(customDateTo);
      const days = Math.ceil((to - from) / (1000 * 60 * 60 * 24)) + 1;
      return Array.from({ length: Math.min(days, 14) }, (_, i) => {
        const d = new Date(from); d.setDate(d.getDate() + i);
        const key = formatDateKey(d);
        const mins = sessions.filter(s => s.date === key && s.endAt).reduce((sum, s) => sum + (s.durationMinutes || 0), 0);
        return { label: `${d.getDate()}`, value: mins, color: COLORS.gold };
      });
    }
    return [];
  };
  ...
  const pieData = chartPeriod === 'today' ? chartData : activeActivities.map(a => {
    ...
    if (chartPeriod === 'week') {
      ...
      for (let i = 0; i < 7; i++) {
        const d = new Date(weekStart); d.setDate(d.getDate() + i);
        const key = formatDateKey(d);
        mins += sessions.filter(s => s.date === key && s.activityId === a.id && s.endAt).reduce((sum, s) => sum + (s.durationMinutes || 0), 0);
      }
    } else if (chartPeriod === 'month') {
    ...
    <div style={{ minHeight: '100vh', background: COLORS.bg, paddingBottom: '100px' }}>
        ...
        <div style={{ background: activeSession ? `${activeActivity?.color}20` : COLORS.bg, borderRadius: '20px', padding: '24px', border: `1px solid ${activeSession ? activeActivity?.color : COLORS.border}`, marginBottom: '20px', textAlign: 'center' }}>
          {activeSession ? (
            <>
              ...
              <button onClick={stopSession} style={{ marginTop: '20px', padding: '14px 32px', background: COLORS.danger, border: 'none', borderRadius: '12px', color: COLORS.text, fontSize: '15px', fontWeight: '600', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: '8px' }}><Square style={{ width: '18px', height: '18px', fill: COLORS.text }} />Стоп</button>
            </>
          ) : (
            <>
      ...
      <div style={{ padding: '20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
          <button onClick={() => setSelectedDate(new Date(selectedDate.setDate(selectedDate.getDate() - 1)))} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><ChevronLeft style={{ width: '20px', height: '20px', color: COLORS.textMuted }} /></button>
          <p style={{ fontSize: '16px', color: COLORS.text, fontWeight: '500' }}>{formatDateDisplay(selectedDate)}</p>
          <button onClick={() => setSelectedDate(new Date(selectedDate.setDate(selectedDate.getDate() + 1)))} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><ChevronRight style={{ width: '20px', height: '20px', color: COLORS.textMuted }} /></button>
        </div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '4px' }}>
                        ...
                        {a.goal > 0 && <span style={{ fontSize: '12px', color: goalReached ? COLORS.success : COLORS.textMuted }}>/ {formatMinutes(a.goal)}</span>}
                      </div>
                    </div>
        ...
        <div style={{ background: COLORS.bgCard, borderRadius: '14px', padding: '16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '4px' }}>
              {CHART_TYPES.map((t) => (
                <button key={t.id} onClick={() => setChartType(t.id)} style={{ padding: '6px', background: chartType === t.id ? `${COLORS.gold}20` : 'transparent', border: `1px solid ${chartType === t.id ? COLORS.gold : COLORS.border}`, borderRadius: '6px', cursor: 'pointer' }}>
                  <t.icon style={{ width: '16px', height: '16px', color: chartType === t.id ? COLORS.gold : COLORS.textMuted }} />
                </button>
              ))}
            </div>
          </div>
          ...
          {chartPeriod === 'custom' && (
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
              <input type="date" value={customDateFrom} onChange={(e) => setCustomDateFrom(e.target.value)} style={{ flex: 1, padding: '8px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '8px', color: COLORS.text, fontSize: '12px' }} />
              <input type="date" value={customDateTo} onChange={(e) => setCustomDateTo(e.target.value)} style={{ flex: 1, padding: '8px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '8px', color: COLORS.text, fontSize: '12px' }} />
            </div>
          )}
            ...
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: COLORS.bg, borderRadius: '12px' }}>
              <button onClick={() => { startSession(a.id); setShowAllActivities(false); }} style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '12px', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  {(a.dailyGoal || a.weeklyGoals) && <p style={{ fontSize: '12px', color: COLORS.textMuted }}>Цель: {a.weeklyGoals ? 'по дням' : formatMinutes(a.dailyGoal)}</p>}
                </div>
              </button>
              <button onClick={() => { setEditingActivity(a); setShowActivityForm(true); setShowAllActivities(false); }} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><Edit3 style={{ width: '16px', height: '16px', color: COLORS.textMuted }} /></button>
            </div>
          ))}
      ...
      <Modal isOpen={showArchive} onClose={() => setShowArchive(false)} title="Архив">
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {archivedActivities.map((a) => (
            <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: COLORS.bg, borderRadius: '12px', opacity: 0.7 }}>
              <div style={{ width: '44px', height: '44px', background: `${a.color}20`, borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '22px' }}>{a.icon}</div>
              <span style={{ flex: 1, fontSize: '15px', color: COLORS.text }}>{a.name}</span>
              <button onClick={() => handleRestoreActivity(a.id)} style={{ padding: '8px 12px', background: `${COLORS.success}20`, border: 'none', borderRadius: '8px', color: COLORS.success, fontSize: '12px', cursor: 'pointer' }}>Восстановить</button>
            </div>
          ))}
      ...
      <Modal isOpen={showActivityForm} onClose={() => { setShowActivityForm(false); setEditingActivity(null); }} title={editingActivity ? 'Редактировать' : 'Новый вид'}>
        <ActivityForm existingActivity={editingActivity} onSave={handleSaveActivity} onClose={() => { setShowActivityForm(false); setEditingActivity(null); }} onDelete={handleDeleteActivity} onArchive={handleArchiveActivity} />
      </Modal>
