...
import { CheckSquare, Plus, Clock, Calendar, List, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, Edit3, Check, X, Trash2, Target, Flag, AlertCircle, ThumbsUp, ThumbsDown, Minus, Repeat, Filter, Grid } from 'lucide-react';
...
const ActionForm = ({ steps, goals, spheres, selectedDate, existingAction, onSave, onClose, onDelete }) => {
  ...
  const handleSave = () => {
    ...
    onSave({
      ...
      priority, strength, stepId: stepId || null, sphereId: autoSphereId || null,
      repeatType, repeatInterval: repeatType === 'custom' ? repeatInterval : null,
      subtasks, status: existingAction?.status || 'active',
    ...
    <div style={{ maxHeight: '70vh', overflowY: 'auto' }}>
      ...
      {spheres.length > 0 && !stepId && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
            {spheres.map((s) => (
              <button key={s.id} onClick={() => setSphereId(s.id)} style={{ padding: '8px 12px', background: sphereId === s.id ? `${s.color}20` : COLORS.bg, border: `1px solid ${sphereId === s.id ? s.color : COLORS.border}`, borderRadius: '20px', color: sphereId === s.id ? s.color : COLORS.textMuted, fontSize: '12px', cursor: 'pointer' }}>{s.icon} {s.name}</button>
            ))}
          </div>
        </div>
      )}
...
// КАРТОЧКА ДЕЙСТВИЯ
// ============================================
const ActionCard = ({ action, step, goal, sphere, isCompleted, onToggle, onEdit }) => {
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '2px' }}>
        ...
        {days.map((d, i) => {
          ...
          return (
            <button key={i} onClick={() => onSelectDate(d.date)} style={{ aspectRatio: '1', padding: '4px', background: isToday ? `${COLORS.gold}20` : 'transparent', border: isToday ? `1px solid ${COLORS.gold}` : '1px solid transparent', borderRadius: '8px', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', opacity: d.isCurrentMonth ? 1 : 0.3 }}>
              <span style={{ fontSize: '13px', color: isToday ? COLORS.gold : COLORS.text }}>{d.date.getDate()}</span>
              {hasActions && <div style={{ width: '4px', height: '4px', background: COLORS.gold, borderRadius: '50%', marginTop: '2px' }} />}
            </button>
          );
        })}
      </div>
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
        {days.map((d, i) => {
          ...
          const dayActions = actions.filter(a => a.date === key && a.status === 'active');
          const isToday = key === today;
          return (
            <div key={i} style={{ background: COLORS.bgCard, borderRadius: '10px', padding: '8px', border: isToday ? `1px solid ${COLORS.gold}` : `1px solid ${COLORS.border}`, minHeight: '100px' }}>
              <div style={{ textAlign: 'center', marginBottom: '8px' }}>
                <span style={{ fontSize: '10px', color: COLORS.textMuted }}>{weekDays[i]}</span>
                <p style={{ fontSize: '16px', fontWeight: '500', color: isToday ? COLORS.gold : COLORS.text }}>{d.getDate()}</p>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                {dayActions.slice(0, 3).map((a) => (
                  <button key={a.id} onClick={() => onSelectDate(d)} style={{ padding: '4px 6px', background: COLORS.bg, borderRadius: '4px', border: 'none', cursor: 'pointer', textAlign: 'left' }}>
                    <span style={{ fontSize: '10px', color: COLORS.text, display: 'block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.title}</span>
                  </button>
                ))}
                {dayActions.length > 3 && <span style={{ fontSize: '10px', color: COLORS.textMuted, textAlign: 'center' }}>+{dayActions.length - 3}</span>}
              </div>
            </div>
          );
        })}
...
export const ActionScreen = ({ data, saveData }) => {
  ...
  const filteredActions = filterGoalId === 'all' ? actions : actions.filter(a => {
    const step = steps.find(s => s.id === a.stepId);
    return step?.goalId === filterGoalId;
  });
  ...
  const handleToggleAction = (action) => {
    ...
    saveData({ ...data, actions: actions.map(a => a.id === action.id ? { ...a, status: 'done', completedAt: new Date().toISOString() } : a) });
  };
  ...
  const getStepGoalSphere = (action) => {
    const step = steps.find(s => s.id === action.stepId);
    const goal = step ? goals.find(g => g.id === step.goalId) : null;
    const sphere = action.sphereId ? spheres.find(s => s.id === action.sphereId) : (goal?.sphereId ? spheres.find(s => s.id === goal.sphereId) : null);
    return { step, goal, sphere };
  };
    ...
    <div style={{ minHeight: '100vh', background: COLORS.bg, paddingBottom: '100px' }}>
      <div style={{ padding: '20px', paddingTop: '60px', background: `linear-gradient(to bottom, ${COLORS.bgCard} 0%, ${COLORS.bg} 100%)` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
          ...
          <button onClick={() => setShowFilter(!showFilter)} style={{ padding: '8px 12px', background: filterGoalId !== 'all' ? `${COLORS.gold}20` : COLORS.bg, border: `1px solid ${filterGoalId !== 'all' ? COLORS.gold : COLORS.border}`, borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer' }}>
            <Filter style={{ width: '16px', height: '16px', color: filterGoalId !== 'all' ? COLORS.gold : COLORS.textMuted }} />
          </button>
        </div>

        {/* Фильтр по целям */}
        {showFilter && (
          <div style={{ marginBottom: '16px', padding: '12px', background: COLORS.bg, borderRadius: '12px', border: `1px solid ${COLORS.border}` }}>
            <p style={{ fontSize: '11px', color: COLORS.textMuted, marginBottom: '8px', textTransform: 'uppercase' }}>Фильтр по цели</p>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              <button onClick={() => setFilterGoalId('all')} style={{ padding: '6px 12px', background: filterGoalId === 'all' ? `${COLORS.gold}20` : 'transparent', border: `1px solid ${filterGoalId === 'all' ? COLORS.gold : COLORS.border}`, borderRadius: '16px', fontSize: '12px', color: filterGoalId === 'all' ? COLORS.gold : COLORS.textMuted, cursor: 'pointer' }}>Все</button>
              {goals.filter(g => g.status === 'active').map(g => (
                <button key={g.id} onClick={() => setFilterGoalId(g.id)} style={{ padding: '6px 12px', background: filterGoalId === g.id ? `${COLORS.gold}20` : 'transparent', border: `1px solid ${filterGoalId === g.id ? COLORS.gold : COLORS.border}`, borderRadius: '16px', fontSize: '12px', color: filterGoalId === g.id ? COLORS.gold : COLORS.text, cursor: 'pointer' }}>{g.title}</button>
              ))}
            </div>
          </div>
        )}
        ...
        <div style={{ display: 'flex', gap: '4px', marginBottom: '16px', background: COLORS.bg, padding: '4px', borderRadius: '10px' }}>
          {VIEW_MODES.map(v => (
            <button key={v.id} onClick={() => setViewMode(v.id)} style={{ flex: 1, padding: '10px', background: viewMode === v.id ? COLORS.bgCard : 'transparent', border: 'none', borderRadius: '8px', color: viewMode === v.id ? COLORS.gold : COLORS.textMuted, fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
              <v.icon style={{ width: '16px', height: '16px' }} />{v.label}
            </button>
          ))}
        </div>
          ...
          <>
            {withTime.length > 0 && (
              <div style={{ marginBottom: '24px' }}>
                ...
                {withTime.map((action) => { const { step, goal, sphere } = getStepGoalSphere(action); return <ActionCard key={action.id} action={action} step={step} goal={goal} sphere={sphere} isCompleted={false} onToggle={() => handleToggleAction(action)} onEdit={() => { setEditingAction(action); setShowForm(true); }} />; })}
              </div>
            )}
            ...
            {withoutTime.length > 0 && (
              <div style={{ marginBottom: '24px' }}>
                ...
                {withoutTime.map((action) => { const { step, goal, sphere } = getStepGoalSphere(action); return <ActionCard key={action.id} action={action} step={step} goal={goal} sphere={sphere} isCompleted={false} onToggle={() => handleToggleAction(action)} onEdit={() => { setEditingAction(action); setShowForm(true); }} />; })}
              </div>
            )}
            ...
            {completedActions.length > 0 && (
              <div style={{ marginBottom: '24px' }}>
                ...
                {completedActions.map((action) => { const { step, goal, sphere } = getStepGoalSphere(action); return <ActionCard key={action.id} action={action} step={step} goal={goal} sphere={sphere} isCompleted={true} onToggle={() => {}} onEdit={() => {}} />; })}
              </div>
            )}
            ...
            {undatedActions.length > 0 && (
              <div>
                ...
                {undatedActions.map((action) => { const { step, goal, sphere } = getStepGoalSphere(action); return <ActionCard key={action.id} action={action} step={step} goal={goal} sphere={sphere} isCompleted={false} onToggle={() => handleToggleAction(action)} onEdit={() => { setEditingAction(action); setShowForm(true); }} />; })}
              </div>
            )}
          </>
        )}
