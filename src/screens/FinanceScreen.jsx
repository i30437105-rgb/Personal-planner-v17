...
import { Wallet, TrendingUp, TrendingDown, Plus, ChevronLeft, ChevronRight, ChevronDown, ChevronUp, PiggyBank, Edit3, Trash2, X, Check, PieChart, BarChart2, Filter, Repeat, Calendar, Target } from 'lucide-react';
...
const getMonthName = (month) => ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'][month];

const getPeriodLabel = (periodType, date) => {
  const d = new Date(date);
  if (periodType === 'month') return `${getMonthName(d.getMonth())} ${d.getFullYear()}`;
...
const getPrevPeriodTransactions = (transactions, periodType, periodDate) => {
  const prevDate = navigatePeriod(periodType, periodDate, -1);
  return transactions.filter(t => isInPeriod(t.date, periodType, prevDate));
};

const getYTDStats = (transactions, periodDate) => {
  const year = new Date(periodDate).getFullYear();
  const yearTxns = transactions.filter(t => new Date(t.date).getFullYear() === year);
  const income = yearTxns.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
  const currentMonth = new Date(periodDate).getMonth() + 1;
  return { total: income, avgMonthly: Math.round(income / currentMonth) };
};
      ...
      <div style={{ position: 'relative', width: '120px', height: '120px', flexShrink: 0 }}>
        <svg viewBox="0 0 32 32" style={{ transform: 'rotate(-90deg)' }}>
          {data.map((item, i) => {
            ...
            return <circle key={i} r="16" cx="16" cy="16" fill="transparent" stroke={item.color} strokeWidth="5" strokeDasharray={strokeDasharray} strokeDashoffset={strokeDashoffset} />;
          })}
        </svg>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', textAlign: 'center' }}>
          <p style={{ fontSize: '12px', fontWeight: '600', color: COLORS.text }}>{formatMoney(total)}</p>
        </div>
      </div>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {data.slice(0, 5).map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
            ...
            <span style={{ fontSize: '11px', color: COLORS.text, flex: 1 }}>{item.name}</span>
            <span style={{ fontSize: '11px', color: COLORS.textMuted }}>{Math.round((item.amount / total) * 100)}%</span>
          </div>
        ))}
      </div>
    </div>
...
const BarChartComponent = ({ data }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: '6px', height: '100px', padding: '10px 0' }}>
      {data.map((item, i) => {
        ...
        return (
          <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
            <div style={{ width: '100%', height: '60px', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
              <div style={{ width: '100%', maxWidth: '30px', height: `${Math.max(height, 3)}%`, background: item.color, borderRadius: '4px 4px 0 0' }} />
            </div>
...
// СРАВНЕНИЕ МЕСЯЦЕВ
// ============================================
const ComparisonChart = ({ current, previous, labels }) => {
  const max = Math.max(current, previous, 1);
  return (
    <div style={{ display: 'flex', gap: '16px', justifyContent: 'center', padding: '10px 0' }}>
      {[{ value: previous, label: labels[0], color: COLORS.textMuted }, { value: current, label: labels[1], color: COLORS.gold }].map((item, i) => (
        <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', width: '80px' }}>
          <div style={{ width: '40px', height: '80px', display: 'flex', alignItems: 'flex-end', background: COLORS.bg, borderRadius: '8px', overflow: 'hidden' }}>
            <div style={{ width: '100%', height: `${(item.value / max) * 100}%`, background: item.color, borderRadius: '8px 8px 0 0', minHeight: '4px' }} />
          </div>
          <span style={{ fontSize: '12px', color: item.color, fontWeight: '500' }}>{formatMoney(item.value)}</span>
    ...
    <div>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {CATEGORY_ICONS.map((i) => <button key={i} onClick={() => setIcon(i)} style={{ width: '40px', height: '40px', background: icon === i ? `${color}30` : COLORS.bg, border: `2px solid ${icon === i ? color : COLORS.border}`, borderRadius: '8px', fontSize: '18px', cursor: 'pointer' }}>{i}</button>)}
        </div>
      </div>
      <div style={{ marginBottom: '20px' }}>
        <label style={labelStyle}>Цвет</label>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {CATEGORY_COLORS.map((c) => <button key={c} onClick={() => setColor(c)} style={{ width: '32px', height: '32px', background: c, border: `3px solid ${color === c ? COLORS.text : 'transparent'}`, borderRadius: '50%', cursor: 'pointer' }} />)}
        </div>
      </div>
      <div style={{ display: 'flex', gap: '12px' }}>
        ...
        <button onClick={() => { if (name.trim()) onSave({ id: existingCategory?.id || `cat_${Date.now()}`, name, icon, color, type }); }} disabled={!name.trim()} style={{ flex: 1, padding: '16px', background: name.trim() ? COLORS.gold : COLORS.bgCard, border: 'none', borderRadius: '12px', color: name.trim() ? COLORS.bg : COLORS.textDark, fontSize: '15px', fontWeight: '600', cursor: name.trim() ? 'pointer' : 'not-allowed' }}>Сохранить</button>
      </div>
    </div>
  );
};
...
// ============================================
const DistributionForm = ({ amount, funds, onSave, onClose }) => {
  const [distribution, setDistribution] = useState(funds.map(f => {
    if (f.ruleType === 'percent') return { fundId: f.id, amount: Math.round((amount * f.ruleValue) / 100) };
    if (f.ruleType === 'fixed') return { fundId: f.id, amount: Math.min(f.ruleValue, amount) };
    return { fundId: f.id, amount: 0 };
  }));

  const totalDistributed = distribution.reduce((s, d) => s + d.amount, 0);
  const remaining = amount - totalDistributed;

  const updateAmount = (fundId, value) => {
    setDistribution(distribution.map(d => d.fundId === fundId ? { ...d, amount: Math.max(0, parseInt(value) || 0) } : d));
  };

  const resetToAuto = () => {
    setDistribution(funds.map(f => {
      if (f.ruleType === 'percent') return { fundId: f.id, amount: Math.round((amount * f.ruleValue) / 100) };
      if (f.ruleType === 'fixed') return { fundId: f.id, amount: Math.min(f.ruleValue, amount) };
      return { fundId: f.id, amount: 0 };
    }));
  ...
  return (
    <div>
      <div style={{ padding: '16px', background: COLORS.bg, borderRadius: '12px', marginBottom: '20px', textAlign: 'center' }}>
        <p style={{ fontSize: '12px', color: COLORS.textMuted }}>Сумма дохода</p>
        <p style={{ fontSize: '24px', color: COLORS.success, fontWeight: '600' }}>{formatMoney(amount)}</p>
      </div>

      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '20px' }}>
        {funds.map(f => {
          const d = distribution.find(x => x.fundId === f.id);
          return (
            <div key={f.id} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px', background: COLORS.bgCard, borderRadius: '12px' }}>
              <span style={{ fontSize: '24px' }}>{f.icon}</span>
              <div style={{ flex: 1 }}>
                <p style={{ fontSize: '14px', color: COLORS.text }}>{f.name}</p>
                <p style={{ fontSize: '11px', color: COLORS.textMuted }}>{f.ruleType === 'percent' ? `${f.ruleValue}%` : f.ruleType === 'fixed' ? formatMoney(f.ruleValue) : 'Вручную'}</p>
              </div>
              <input type="number" value={d?.amount || 0} onChange={(e) => updateAmount(f.id, e.target.value)} style={{ width: '100px', padding: '10px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '8px', color: COLORS.text, fontSize: '14px', textAlign: 'right' }} />
            </div>
          );
        })}
      </div>

      <div style={{ padding: '12px', background: remaining < 0 ? `${COLORS.danger}20` : COLORS.bg, borderRadius: '12px', marginBottom: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '14px', color: COLORS.textMuted }}>Остаток</span>
        <span style={{ fontSize: '16px', color: remaining < 0 ? COLORS.danger : COLORS.text, fontWeight: '500' }}>{formatMoney(remaining)}</span>
      </div>

      <div style={{ display: 'flex', gap: '12px' }}>
        <button onClick={resetToAuto} style={{ padding: '14px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '12px', color: COLORS.textMuted, fontSize: '13px', cursor: 'pointer' }}>Сбросить</button>
        <button onClick={onClose} style={{ flex: 1, padding: '14px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '12px', color: COLORS.text, fontSize: '15px', cursor: 'pointer' }}>Отмена</button>
        <button onClick={() => onSave(distribution)} disabled={remaining < 0} style={{ flex: 1, padding: '14px', background: remaining >= 0 ? COLORS.gold : COLORS.bgCard, border: 'none', borderRadius: '12px', color: remaining >= 0 ? COLORS.bg : COLORS.textDark, fontSize: '15px', fontWeight: '600', cursor: remaining >= 0 ? 'pointer' : 'not-allowed' }}>Применить</button>
      </div>
    </div>
  );
};
...
// ФОРМА ТРАНЗАКЦИИ
// ============================================
const TransactionForm = ({ categories, funds, existingTransaction, onSave, onClose, onDelete, onManageCategories, onDistribute }) => {
  ...
  const handleSave = () => {
    ...
    onSave({ id: existingTransaction?.id || `txn_${Date.now()}`, type, amount: parseFloat(amount), categoryId, comment, date, fundId: type === 'expense' ? fundId : null, isRecurring, createdAt: existingTransaction?.createdAt || new Date().toISOString() });
  };
  ...
  return (
    <div style={{ maxHeight: '70vh', overflowY: 'auto' }}>
      ...
      <div style={{ marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {filteredCategories.map((c) => (
            <button key={c.id} onClick={() => setCategoryId(c.id)} style={{ padding: '10px 14px', background: categoryId === c.id ? `${c.color}20` : COLORS.bg, border: `1px solid ${categoryId === c.id ? c.color : COLORS.border}`, borderRadius: '20px', color: categoryId === c.id ? c.color : COLORS.textMuted, fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}><span>{c.icon}</span>{c.name}</button>
          ))}
        </div>
      </div>
      ...
      {type === 'expense' && funds.length > 0 && (
        <div style={{ marginBottom: '20px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
            ...
            {funds.map((f) => (
              <button key={f.id} onClick={() => setFundId(f.id)} style={{ padding: '10px 14px', background: fundId === f.id ? `${COLORS.gold}20` : COLORS.bg, border: `1px solid ${fundId === f.id ? COLORS.gold : COLORS.border}`, borderRadius: '20px', color: fundId === f.id ? COLORS.gold : COLORS.textMuted, fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px' }}><span>{f.icon}</span>{f.name}</button>
            ))}
          </div>
        </div>
      )}
      ...
      {type === 'income' && funds.length > 0 && amount && (
        <button onClick={() => onDistribute(parseFloat(amount))} style={{ width: '100%', padding: '12px', background: `${COLORS.gold}15`, border: `1px solid ${COLORS.gold}30`, borderRadius: '12px', color: COLORS.gold, fontSize: '13px', cursor: 'pointer', marginBottom: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
          <PiggyBank style={{ width: '16px', height: '16px' }} />Настроить распределение по фондам
        </button>
      )}
      ...
      <button onClick={() => setIsRecurring(!isRecurring)} style={{ width: '100%', padding: '14px', background: isRecurring ? `${COLORS.gold}20` : COLORS.bg, border: `1px solid ${isRecurring ? COLORS.gold : COLORS.border}`, borderRadius: '12px', color: isRecurring ? COLORS.gold : COLORS.textMuted, fontSize: '14px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '20px' }}>
        <Repeat style={{ width: '18px', height: '18px' }} />Повторять ежемесячно
      </button>
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: '20px' }}>
        <label style={labelStyle}>Иконка</label>
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          {FUND_ICONS.map((i) => <button key={i} onClick={() => setIcon(i)} style={{ width: '44px', height: '44px', background: icon === i ? `${COLORS.gold}30` : COLORS.bg, border: `2px solid ${icon === i ? COLORS.gold : COLORS.border}`, borderRadius: '10px', fontSize: '20px', cursor: 'pointer' }}>{i}</button>)}
        </div>
      </div>
      <div style={{ marginBottom: '20px' }}>
        <label style={labelStyle}>Правило пополнения</label>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          {[{ id: 'percent', label: '% от дохода' }, { id: 'fixed', label: 'Фикс. сумма' }, { id: 'choice', label: 'Вручную' }].map(r => (
            <button key={r.id} onClick={() => setRuleType(r.id)} style={{ flex: 1, padding: '12px', background: ruleType === r.id ? `${COLORS.gold}20` : COLORS.bg, border: `1px solid ${ruleType === r.id ? COLORS.gold : COLORS.border}`, borderRadius: '10px', color: ruleType === r.id ? COLORS.gold : COLORS.textMuted, fontSize: '13px', cursor: 'pointer' }}>{r.label}</button>
          ))}
        </div>
      ...
      <div style={{ display: 'flex', gap: '12px' }}>
        ...
        <button onClick={onClose} style={{ flex: 1, padding: '16px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '12px', color: COLORS.text, fontSize: '15px', cursor: 'pointer' }}>Отмена</button>
        <button onClick={() => { if (name.trim()) onSave({ id: existingFund?.id || `fund_${Date.now()}`, name, icon, ruleType, ruleValue: ruleValue ? parseFloat(ruleValue) : null, balance: existingFund?.balance || 0, createdAt: existingFund?.createdAt || new Date().toISOString() }); }} disabled={!name.trim()} style={{ flex: 1, padding: '16px', background: name.trim() ? COLORS.gold : COLORS.bgCard, border: 'none', borderRadius: '12px', color: name.trim() ? COLORS.bg : COLORS.textDark, fontSize: '15px', fontWeight: '600', cursor: name.trim() ? 'pointer' : 'not-allowed' }}>Сохранить</button>
      </div>
    </div>
  );
};
...
// БЮДЖЕТ
// ============================================
const BudgetView = ({ categories, transactions, budgets, periodDate, onSaveBudget }) => {
  const expenseCategories = categories.filter(c => c.type === 'expense');
  const quarter = Math.floor(new Date(periodDate).getMonth() / 3);
  const year = new Date(periodDate).getFullYear();
  const months = [quarter * 3, quarter * 3 + 1, quarter * 3 + 2];

  const getMonthFact = (categoryId, monthIndex) => {
    return transactions.filter(t => {
      const d = new Date(t.date);
      return t.categoryId === categoryId && d.getMonth() === monthIndex && d.getFullYear() === year;
    }).reduce((s, t) => s + t.amount, 0);
  };

  const getLimit = (categoryId, monthIndex) => {
    const key = `${categoryId}_${year}_${monthIndex}`;
    return budgets?.[key] || 0;
  };

  const setLimit = (categoryId, monthIndex, value) => {
    const key = `${categoryId}_${year}_${monthIndex}`;
    onSaveBudget({ ...budgets, [key]: parseInt(value) || 0 });
  };

  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '16px' }}>
        <span style={{ fontSize: '14px', color: COLORS.text, fontWeight: '500' }}>Q{quarter + 1} {year}</span>
      </div>
      <div style={{ overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr>
              <th style={{ padding: '8px', textAlign: 'left', fontSize: '11px', color: COLORS.textMuted }}>Категория</th>
              {months.map(m => <th key={m} style={{ padding: '8px', textAlign: 'center', fontSize: '11px', color: COLORS.textMuted }}>{getMonthName(m).slice(0, 3)}</th>)}
            </tr>
          </thead>
          <tbody>
            {expenseCategories.map(cat => (
              <tr key={cat.id}>
                <td style={{ padding: '8px', fontSize: '13px', color: COLORS.text }}>{cat.icon} {cat.name}</td>
                {months.map(m => {
                  const fact = getMonthFact(cat.id, m);
                  const limit = getLimit(cat.id, m);
                  const over = limit > 0 && fact > limit;
                  return (
                    <td key={m} style={{ padding: '8px', textAlign: 'center' }}>
                      <input type="number" value={limit || ''} onChange={(e) => setLimit(cat.id, m, e.target.value)} placeholder="0" style={{ width: '60px', padding: '4px', background: COLORS.bg, border: `1px solid ${COLORS.border}`, borderRadius: '4px', color: COLORS.textMuted, fontSize: '11px', textAlign: 'center', marginBottom: '4px' }} />
                      <div style={{ fontSize: '12px', color: over ? COLORS.danger : COLORS.text, fontWeight: over ? '600' : '400' }}>{formatMoney(fact)}</div>
                    </td>
                  );
                })}
              </tr>
...
// ГРУППА КАТЕГОРИЙ
// ============================================
const CategoryGroup = ({ group, onEditTransaction }) => {
  const [expanded, setExpanded] = useState(false);
  return (
    <div style={{ background: COLORS.bgCard, borderRadius: '12px', border: `1px solid ${COLORS.border}`, overflow: 'hidden', marginBottom: '8px' }}>
          ...
          {group.transactions.map((t) => (
            <button key={t.id} onClick={() => onEditTransaction(t)} style={{ width: '100%', padding: '10px', display: 'flex', justifyContent: 'space-between', background: 'none', border: 'none', cursor: 'pointer', borderRadius: '8px' }}>
              <div style={{ textAlign: 'left' }}>
                ...
                <p style={{ fontSize: '11px', color: COLORS.textMuted }}>{new Date(t.date).toLocaleDateString('ru-RU')}{t.isRecurring && <Repeat style={{ width: '10px', height: '10px', marginLeft: '6px', verticalAlign: 'middle' }} />}</p>
              </div>
...
export const FinanceScreen = ({ data, saveData }) => {
  ...
  useEffect(() => {
    if (!data.financeCategories) {
      saveData({ ...data, financeCategories: [...DEFAULT_INCOME_CATEGORIES, ...DEFAULT_EXPENSE_CATEGORIES], funds: DEFAULT_FUNDS, transactions: [], budgets: {} });
    }
  }, [data, saveData]);
  ...
  const filteredTransactions = filterCategoryId === 'all' ? periodTransactions : periodTransactions.filter(t => t.categoryId === filterCategoryId);
  ...
  const prevPeriodTxns = getPrevPeriodTransactions(transactions, periodType, periodDate);
  const prevIncome = prevPeriodTxns.filter(t => t.type === 'income').reduce((s, t) => s + t.amount, 0);
  const prevExpense = prevPeriodTxns.filter(t => t.type === 'expense').reduce((s, t) => s + t.amount, 0);
  ...
  const ytdStats = getYTDStats(transactions, periodDate);
  ...
  const getGroupedByCategory = (type) => {
    ...
    return Object.entries(grouped).map(([catId, d]) => {
  ...
  const handleSaveTransaction = (transaction) => {
    ...
    if (transaction.type === 'income' && existingIndex === -1) {
      if (customDistribution) {
        newFunds = funds.map(f => {
          const dist = customDistribution.find(d => d.fundId === f.id);
          return dist ? { ...f, balance: f.balance + dist.amount } : f;
        });
        setCustomDistribution(null);
      } else {
        newFunds = funds.map(f => {
          let addition = 0;
          if (f.ruleType === 'percent' && f.ruleValue) addition = Math.round((transaction.amount * f.ruleValue) / 100);
  ...
  const chartData = getGroupedByCategory(detailsType).map(g => ({ name: g.name, icon: g.icon, color: g.color, amount: g.total }));
    ...
    <div style={{ minHeight: '100vh', background: COLORS.bg, paddingBottom: '100px' }}>
      <div style={{ padding: '20px', paddingTop: '60px', background: `linear-gradient(to bottom, ${COLORS.bgCard} 0%, ${COLORS.bg} 100%)` }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <button onClick={() => setPeriodDate(navigatePeriod(periodType, periodDate, -1))} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><ChevronLeft style={{ width: '24px', height: '24px', color: COLORS.textMuted }} /></button>
          <h1 style={{ fontSize: '20px', fontWeight: '600', color: COLORS.text, fontFamily: 'Georgia, serif' }}>{getPeriodLabel(periodType, periodDate)}</h1>
          <button onClick={() => setPeriodDate(navigatePeriod(periodType, periodDate, 1))} style={{ padding: '8px', background: 'none', border: 'none', cursor: 'pointer' }}><ChevronRight style={{ width: '24px', height: '24px', color: COLORS.textMuted }} /></button>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
          <button onClick={() => { setDetailsType('income'); setShowDetails(true); }} style={{ flex: 1, padding: '16px', background: `${COLORS.success}15`, borderRadius: '16px', border: `1px solid ${COLORS.success}30`, cursor: 'pointer', textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              ...
              {incomeTrend !== 0 && <span style={{ fontSize: '10px', color: incomeTrend > 0 ? COLORS.success : COLORS.danger, marginLeft: 'auto' }}>{incomeTrend > 0 ? '+' : ''}{incomeTrend}%</span>}
            </div>
          ...
          <button onClick={() => { setDetailsType('expense'); setShowDetails(true); }} style={{ flex: 1, padding: '16px', background: `${COLORS.danger}15`, borderRadius: '16px', border: `1px solid ${COLORS.danger}30`, cursor: 'pointer', textAlign: 'left' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
              ...
              {expenseTrend !== 0 && <span style={{ fontSize: '10px', color: expenseTrend > 0 ? COLORS.danger : COLORS.success, marginLeft: 'auto' }}>{expenseTrend > 0 ? '+' : ''}{expenseTrend}%</span>}
            </div>
        ...
        {/* YTD */}
        <div style={{ display: 'flex', gap: '12px' }}>
          <div style={{ flex: 1, padding: '12px', background: COLORS.bg, borderRadius: '10px', border: `1px solid ${COLORS.border}` }}>
            <p style={{ fontSize: '10px', color: COLORS.textMuted, marginBottom: '4px' }}>Доход YTD</p>
            <p style={{ fontSize: '14px', color: COLORS.success, fontWeight: '500' }}>{formatMoney(ytdStats.total)}</p>
          </div>
          <div style={{ flex: 1, padding: '12px', background: COLORS.bg, borderRadius: '10px', border: `1px solid ${COLORS.border}` }}>
            <p style={{ fontSize: '10px', color: COLORS.textMuted, marginBottom: '4px' }}>Средний/мес</p>
            <p style={{ fontSize: '14px', color: COLORS.text, fontWeight: '500' }}>{formatMoney(ytdStats.avgMonthly)}</p>
          </div>
          <button onClick={() => setShowBudget(true)} style={{ flex: 1, padding: '12px', background: `${COLORS.gold}15`, borderRadius: '10px', border: `1px solid ${COLORS.gold}30`, cursor: 'pointer' }}>
            <Target style={{ width: '16px', height: '16px', color: COLORS.gold, margin: '0 auto 4px', display: 'block' }} />
            <p style={{ fontSize: '10px', color: COLORS.gold }}>Бюджет</p>
          </button>
        </div>
      </div>
      ...
      <div style={{ padding: '20px' }}>
        {/* Фонды */}
        <button onClick={() => setFundsExpanded(!fundsExpanded)} style={{ width: '100%', padding: '14px', background: `${COLORS.gold}15`, borderRadius: '12px', border: `1px solid ${COLORS.gold}30`, marginBottom: fundsExpanded ? '12px' : '24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', cursor: 'pointer' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={() => setShowFilters(!showFilters)} style={{ padding: '6px', background: filterCategoryId !== 'all' ? `${COLORS.gold}20` : 'transparent', border: `1px solid ${filterCategoryId !== 'all' ? COLORS.gold : COLORS.border}`, borderRadius: '6px', cursor: 'pointer' }}><Filter style={{ width: '16px', height: '16px', color: filterCategoryId !== 'all' ? COLORS.gold : COLORS.textMuted }} /></button>
        ...
        {showFilters && (
          <div style={{ marginBottom: '16px', padding: '12px', background: COLORS.bgCard, borderRadius: '12px' }}>
            <p style={{ fontSize: '11px', color: COLORS.textMuted, marginBottom: '8px' }}>Фильтр по категории</p>
            <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
              <button onClick={() => setFilterCategoryId('all')} style={{ padding: '6px 10px', background: filterCategoryId === 'all' ? `${COLORS.gold}20` : COLORS.bg, border: `1px solid ${filterCategoryId === 'all' ? COLORS.gold : COLORS.border}`, borderRadius: '16px', fontSize: '11px', color: filterCategoryId === 'all' ? COLORS.gold : COLORS.textMuted, cursor: 'pointer' }}>Все</button>
              {categories.map(c => (
                <button key={c.id} onClick={() => setFilterCategoryId(c.id)} style={{ padding: '6px 10px', background: filterCategoryId === c.id ? `${c.color}20` : COLORS.bg, border: `1px solid ${filterCategoryId === c.id ? c.color : COLORS.border}`, borderRadius: '16px', fontSize: '11px', color: filterCategoryId === c.id ? c.color : COLORS.textMuted, cursor: 'pointer' }}>{c.icon}</button>
              ))}
            </div>
          </div>
        )}
        ...
        {filteredTransactions.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {filteredTransactions.slice(-10).reverse().map((t) => {
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    <div style={{ textAlign: 'left' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                        {t.comment && <p style={{ fontSize: '11px', color: COLORS.textMuted }}>{t.comment}</p>}
                        {t.isRecurring && <Repeat style={{ width: '10px', height: '10px', color: COLORS.textMuted }} />}
                      </div>
                    </div>
                  </div>
      ...
      <Modal isOpen={showTransactionForm} onClose={() => { setShowTransactionForm(false); setEditingTransaction(null); }} title={editingTransaction ? 'Редактировать' : 'Новая операция'}>
        <TransactionForm categories={categories} funds={funds} existingTransaction={editingTransaction} onSave={handleSaveTransaction} onClose={() => { setShowTransactionForm(false); setEditingTransaction(null); }} onDelete={handleDeleteTransaction} onManageCategories={(type) => { setCategoryFormType(type); setShowCategoryManager(true); }} onDistribute={(amount) => { setDistributionAmount(amount); setShowDistribution(true); }} />
      </Modal>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {categories.filter(c => c.type === categoryFormType).map((c) => (
            <div key={c.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px', background: COLORS.bg, borderRadius: '10px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}><span style={{ fontSize: '20px' }}>{c.icon}</span><span style={{ fontSize: '14px', color: COLORS.text }}>{c.name}</span></div>
      ...
      <Modal isOpen={showDistribution} onClose={() => setShowDistribution(false)} title="Распределение по фондам">
        <DistributionForm amount={distributionAmount} funds={funds} onSave={(dist) => { setCustomDistribution(dist); setShowDistribution(false); }} onClose={() => setShowDistribution(false)} />
      </Modal>

      <Modal isOpen={showBudget} onClose={() => setShowBudget(false)} title="Бюджет квартала">
        <BudgetView categories={categories} transactions={transactions} budgets={budgets} periodDate={periodDate} onSaveBudget={handleSaveBudget} />
      </Modal>
      ...
      <Modal isOpen={showDetails} onClose={() => setShowDetails(false)} title="Подробно">
        <div style={{ maxHeight: '70vh', overflowY: 'auto' }}>
          ...
          <div style={{ display: 'flex', gap: '4px', marginBottom: '16px' }}>
            {CHART_VIEWS.map((v) => <button key={v.id} onClick={() => setChartView(v.id)} style={{ flex: 1, padding: '8px', background: chartView === v.id ? `${COLORS.gold}20` : COLORS.bg, border: `1px solid ${chartView === v.id ? COLORS.gold : COLORS.border}`, borderRadius: '8px', color: chartView === v.id ? COLORS.gold : COLORS.textMuted, fontSize: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '4px' }}><v.icon style={{ width: '14px', height: '14px' }} />{v.label}</button>)}
          </div>

          <div style={{ marginBottom: '20px' }}>
            {chartView === 'pie' ? <PieChartComponent data={chartData} total={detailsType === 'expense' ? totalExpense : totalIncome} /> : <BarChartComponent data={chartData} />}
          </div>

          <div style={{ marginBottom: '16px' }}>
            <p style={{ fontSize: '12px', color: COLORS.textMuted, marginBottom: '8px' }}>Сравнение с прошлым периодом</p>
            <ComparisonChart current={detailsType === 'expense' ? totalExpense : totalIncome} previous={detailsType === 'expense' ? prevExpense : prevIncome} labels={['Прошлый', 'Текущий']} />
          </div>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {getGroupedByCategory(detailsType).map((group) => <CategoryGroup key={group.id} group={group} onEditTransaction={(t) => { setEditingTransaction(t); setShowTransactionForm(true); setShowDetails(false); }} />)}
          </div>
        </div>
      </Modal>
    </div>
